...
if doPlot
    ...
    plot(1:nFrames, squeeze(max(max(v, [], 1), [], 2)), 'k', 'linewidth', 2);
    hvert = plot([0, 0], ylim, 'r-', 'linewidth', 2);
    title('max temperature in frame');
    xlabel('time index');
    
    subplot(3, 1, 3); hold on;
    plot(1:nFrames, squeeze(sum(sum(fullMask,  1), 2)), 'k', 'linewidth', 2);
    hvert2 = plot([0, 0], ylim, 'r-', 'linewidth', 2);
    title('# detected foreground pixels');
    xlabel('time index');
